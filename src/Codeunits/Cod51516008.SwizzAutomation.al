...
Codeunit 51516008 SwizzAutomation
{
    ...
    var
        ...
        Stos: Record 51516449;
        DocNo: Code[10];
        PDate: Date;
        Transfer: Boolean;
        objFDProcess: Record 51516435;
    ...
    begin
        ...
        if objTransactions.Find('-') then begin
            repeat
                if Account.Get(objTransactions."Account No") then begin
                    if Account."Cheque Discounted" <> 0 then begin
                        LineNo := LineNo + 10000;
                        GenSetup.Get();
                        GenJournalLine.Init;
        ...
        if objFDProcess.Find('-') then begin
            repeat
                ...
                if objFDProcess."FD Maturity Date" <= Today then begin
                    if objFDProcess."Untranfered Interest" > 0 then begin
                        if AccountType.Get(objFDProcess."Account Type") then begin
                            if AccountType."Fixed Deposit" = true then begin
                                if objFDProcess."FD Maturity Date" <= PostingDate then begin
                        ...
                        if Transfer = true then begin
                            ...
                            if objFDProcess."FD Maturity Date" <= Today then begin
                                ...
                                if AmountToTransfer > 0 then begin
                                    //A.-------------------DEBIT FD A/C---------------------------------------------------------
                                    LineNo := LineNo + 10000;
                                ...
                                if AmountToRollBack > 0 then begin
                                    //A.-------------------DEBIT FOSA A/C---------------------------------------------------------
                                    LineNo := LineNo + 10000;
                                    SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
    ...
    local procedure FnGenerateNewSavingsTransaction(ObjFD: Record 51516435; RollBackAmount: Decimal; PreviousPostingDate: Date)
    var
        ObjNewFD: Record 51516435;
    begin
        ObjNewFD.Init;
