...
Table 51516031 "Funds User Setup"
{
    ...
    fields
    {
        ...
        field(82; "Salaries Template"; Code[20])
        {
            TableRelation = "Gen. Journal Template".Name where(Type = const(General));
        }
        field(83; "Salaries Batch"; Code[20])
        {
        ...
        field(84; "Pension Template"; Code[20])
        {
            TableRelation = "Gen. Journal Template".Name where(Type = const(General));
        }
        field(85; "Pension Batch"; Code[20])
        {
            TableRelation = "Gen. Journal Batch".Name where("Journal Template Name" = field("Pension Template"));

            trigger OnValidate()
            begin
                UserTemp.Reset;
                UserTemp.SetRange(UserTemp."Pension Template", "Pension Template");
                UserTemp.SetRange(UserTemp."Pension Batch", "Pension Batch");
                if UserTemp.FindFirst then begin
                    repeat
                        if (UserTemp.UserID <> Rec.UserID) and ("Pension Batch" <> '') then begin
                            Error(SameBatch, "Receipt Journal Batch", "Pension Batch");
                        end;
                    until UserTemp.Next = 0;
                end;
            end;
        }
        field(86; "Board PVs Nos."; code[20]) { }
        field(87; "Imprest Req No"; code[20]) { }
    }
