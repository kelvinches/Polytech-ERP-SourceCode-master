...
Codeunit 51516123 BackOfficePORTAL
{
    ...
    begin
        ...
        //MESSAGE(fnGetLoanNumber('2281'));
        SMSMessage('PORTALTRAN', '123', '0705265017', '1234 Is ur OTP');
        ...
        //MESSAGE(FORMAT(FnFirstTime('2281', '25208538','1234')));
        //MESSAGE(FORMAT(FnCreatePassword('2281','11723','1234')));
    end;
    ...
    var
        ...
        objAtmapplication: Record 51516464;
        objRegMember: Record 51516360;
        objNextKin: Record 51516366;
        GenSetup: Record 51516398;
        FreeShares: Decimal;
        glamount: Decimal;
        LoansGuaranteeDetails: Record 51516372;
        ...
        FAccNo: Text[250];
        sms: Text[250];
        objLoanApplication: Record 51516562;
        ClientName: Code[20];
        Loansetup: Record 51516381;
        LoansPurpose: Record 51516378;
...
;
    ...
    begin
        objLoanRegister.Reset;
        objLoanRegister.SetRange("Staff No", MemberNo);
        ...
        if Loansetup.Find('-') then begin
            ...
            if Loansetup."Repayment Method" = Loansetup."repayment method"::"Straight Line" then begin
                ...
                repeat
                    ...
                    LInterest := (Loansetup."Interest rate" / 12 / 100) * LoanAmount;
                    TotalMRepay := LPrincipal + LInterest;
                    LoanAmount := LoanAmount - LPrincipal;
                    RepayPeriod := RepayPeriod - 1;
                    iEntryNo := iEntryNo + 1;

                    text := text + Format(iEntryNo) + '!!' + Format(ROUND(LPrincipal)) + '!!' + Format(ROUND(LInterest)) + '!!' + Format(ROUND(TotalMRepay)) + '!!' + Format(ROUND(LoanAmount)) + '??';
                until RepayPeriod = 0;
                Message(text);
            end;
            ...
            if Loansetup."Repayment Method" = Loansetup."repayment method"::"Reducing Balance" then begin
                ...
                repeat
                    ...
                    LInterest := ROUND(LoanAmount * Loansetup."Interest rate" / 12 / 100);
                    LoanAmount := LoanAmount - LPrincipal;
                    RepayPeriod := RepayPeriod - 1;
                    TotalMRepay := LPrincipal + LInterest;
                    iEntryNo := iEntryNo + 1;
    ...
    begin
        objLoanRegister.Reset;
        objLoanRegister.SetRange("Staff No", Member);
        if objLoanRegister.Find('-') then begin
            objLoanRegister.SetCurrentkey("Application Date");
    ...
    begin
        ...
        // DivProg.Date:=ToDate;
        // DivProg."Gross Dividends":=CDiv;
        // DivProg."Witholding Tax":=CDiv*(GenSetup."Withholding Tax (%)"/100);fst
        // DivProg."Net Dividends":=DivProg."Gross Dividends"-DivProg."Witholding Tax";
    ...
    begin
        //result:=SwizzKashLive.FundsTransferFOSA(Acountfrom, AccountTo, DocNo, Amount);
    end;
    ...
    begin
        ...
        if objAtmapplication.Find('-') then begin
            repeat
                return := objAtmapplication."Account No" + ':::' + Format(objAtmapplication."Document No") + ':::' +/*FORMAT(objAtmapplication.Status)+':::'+FORMAT(objAtmapplication."Terms Read and Understood")+*/'::::' + return;
            until
    ...
    begin
        ...
        if objLoanRegister.Find('-') then begin
            repeat
                ...
                //objLoanRegister.SETRANGE(objLoanRegister."Outstanding Balance">0);
                if objLoanRegister."Outstanding Balance" > 0 then begin
    ...
    procedure FnCreatePassword(MemberNo: Code[20]; OTP: Code[10]; NewPassword: Text) updated: Boolean
    begin
    ...
    procedure FnFirstTime(memberNo: Code[10]; IDNo: Code[10]; OTP: Code[10]) Response: Boolean
    begin
