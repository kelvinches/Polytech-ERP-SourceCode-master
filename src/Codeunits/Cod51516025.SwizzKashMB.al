...
Codeunit 51516025 SwizzKashMB
{
    ...
    var
        ...
        ExcDuty: Decimal;
        TempBalance: Decimal;
        BOSATransSchedule: Record 51516375;
        ...
        varLoan: Text[500];
        CoopbankTran: Record 51516552;
        loanamt: Decimal;
        description: Code[100];
        hlamount: Decimal;
        commision: Decimal;
        Mstatus: Code[10];
        SaccoGenSetup: Record 51516398;
        MpesaAccount: Code[50];
        MpesaDisbus: Record 51516094;
        MPESACharge: Decimal;
        TariffDetails: Record 51516564;
        GenSetUp: Record 51516398;
        NoSeriesMgt: Codeunit NoSeriesManagement;
        VarReceivableAccount: Code[20];
        SFactory: Codeunit "Swizzsoft Factory.";
        ObjIprsLogs: Record 51516422;
        LoanRepay: Record 51516375;
        Mrowcount: Integer;
                ...
                //CR ACC2
                        ...
                        result:='TRUE';
                           ...
                           msg:='You have transfered KES '+Format(amount)+' from Account '+Vendor.Name+' to '+accTo+
                            ' .Thank you for POLYTECH Sacco Mobile.';
                            SMSMessage(DocNumber,accFrom,Vendor."Phone No.",msg,'');
                     end
                     else begin
                     result:='INSUFFICIENT';
          ...
          if Members.Find('-') then begin
              ...
              if MemberLedgerEntry.Find('-') then
                repeat
                    ...
                    SMSMessage(DocNumber,Members."No.",Phone,' Your Account balance is Kshs: '+Bal+' Thank you for using POLYTECH SACCO Mobile','');
                end
                else
         ...
         //Post
            ...
            if GenJournalLine.Find('-') then begin
              ...
              msg:='Dear ' +Members.Name+' your: '+PaybillTrans."Account No"+' has been credited with Ksh'+ Format(amount) +'. Your new balance is '+
                        'Ksh. '+Format(TempBalance)+'. Thank you for using Polytech Mobile Services';
              SMSMessage(docNo,Members."No.",Members."Mobile Phone No",msg,'');
            end else begin
              PaybillTrans."Date Posted":=Today;
              PaybillTrans."Needs Manual Posting":=true;
    ...
    begin
        ...
        // =================================== Check if member has an outstanding ELOAN
                  ...
                  if (LoansRegister."Loan Product Type" = '24') then begin
                    amount:=2;
                    Res:='2::::Your do not Qualify for this loan because You have an outstanding M-POLYTECH Loan::::False';
                    exit;
                  end;
                end;
    ...
    begin
        ...
        */
        ...
        //Cr Interest Eloan
                  ...
                  GenJournalLine."External Document No.":=docNo;
                  GenJournalLine."Posting Date":=Today;
                  GenJournalLine.Description:=' M-POLYTECH -'+LoansRegister."Loan  No.";
                  GenJournalLine.Amount:=amount*-1;
                  GenJournalLine.Validate(GenJournalLine.Amount);
                  if GenJournalLine.Amount<>0 then
                  GenJournalLine.Insert;
                  ...
                  GenJournalLine."External Document No.":=docNo;
                  GenJournalLine."Posting Date":=Today;
                  GenJournalLine.Description:=' M-POLYTECH -'+LoansRegister."Loan  No." +' - Charges';
        ...
        //=====================insert to Mpesa mobile disbursment
                  ...
                  SurePESATrans.Insert;
                  result:='TRUE';
                  msg:='Dear '+SplitString(Members.Name,' ')+', Your M-POLYTECH LOAN '+loanNo+' of Ksh '+Format((AmountDispursed-TotalCharges))+' has been approved and disbursed to your MPESA. Transaction charges '+Format(TotalCharges)
                ...
                if Today>= CalcDate('1M+1D',VarIssuedDate)   then begin
                 ...
                 if Today>= CalcDate('1M+3D',VarExpectedCompletion)   then begin // recover from deposit
                  ...
                  GLPosting.Run(GenJournalLine);
                   ...
                   +' has been recovered and you have been penalized from accessing the M-POLYTECH loan product ';
    ...
    var
        membersTable: Record 51516364;
        memberledgerentryTable: Record 51516365;
        vendorTable: Record Vendor;
        amount: Decimal;
    begin
    ...
    var
        ...
        loanProductsTable: Record 51516381;
    begin
