...
Table 51516057 "Pending Vch. Surr. Line"
{
    ...
    fields
    {
        ...
        {
            TableRelation = if ("Type Of Surrender"=const(Receipt)) "G/L Account"."No." where ("Direct Posting"=const(true)) else if ("Type Of Surrender"=const(Cash)) Logos.Code;

            trigger OnValidate()
            begin
                //check the type of account selected by the system user
                if "Type Of Surrender"="type of surrender"::Receipt then
                  begin
                    TestField("Account No.");
                    Acc.Reset;
                    if Acc.Get("Account No.") then
                      begin
