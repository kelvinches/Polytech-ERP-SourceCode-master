...
Codeunit 55484 "Budgetary Control"
{
    ...
    begin
        ...
        if BCSetup.Mandatory then//budgetary control is mandatory
          begin
            ...
            else if (PurchHeader."Document Date" > BCSetup."Current Budget End Date") then begin
                Error('The Current Date %1 In The Order Does Not Fall Within Budget Dates %2 - %3', PurchHeader."Document Date",
                BCSetup."Current Budget Start Date", BCSetup."Current Budget End Date");
    ...
    procedure CheckImprest(var ImprestHeader: Record 51516006)
    var
        PayLine: Record 51516007;
        Commitments: Record 51516036;
        Amount: Decimal;
        GLAcc: Record "G/L Account";
        Item: Record Item;
    ...
    begin
        ...
        if BCSetup.Mandatory then//budgetary control is mandatory
          begin
            ...
            else if (ImprestHeader.Date > BCSetup."Current Budget End Date") then begin
                Error('The Current Date %1 for the Imprest Does Not Fall Within Budget Dates %2 - %3', ImprestHeader.Date,
                BCSetup."Current Budget Start Date", BCSetup."Current Budget End Date");
    ...
    var
        Commitments: Record 51516036;
        EntryNo: Integer;
        CommittedLines: Record 51516036;
    begin
        //Get Commitment Lines
        Commitments.Reset;
        if Commitments.Find('+') then
    ...
    var
        ...
        BudgetaryControl: Record 51516038;
    ...
    procedure CheckStaffClaim(var ImprestHeader: Record 51516010)
    var
    ...
    procedure CheckStaffAdvance(var ImprestHeader: Record 51516006)
    var
        PayLine: Record 51516007;
        Commitments: Record 51516036;
        Amount: Decimal;
    ...
    procedure CheckStaffAdvSurr(var ImprestHeader: Record 51516497)
    var
        PayLine: Record 51516498;
        Commitments: Record 51516440;
        Amount: Decimal;
        GLAcc: Record "G/L Account";
        Item: Record Item;
        FirstDay: Date;
    ...
    procedure CheckGrantSurr(var GrantHeader: Record 51516219)
    var
        GrantLine: Record 51516220;
        Commitments: Record 51516440;
        Amount: Decimal;
        GLAcc: Record "G/L Account";
        Item: Record Item;
